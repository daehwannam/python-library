...
# reference: https://gist.github.com/kilian-gebhardt/6f1db877797d69fa1df6aa936feea607
...
def level(i):
    return (i+1).bit_length() - 1  # math.floor(math.log(i, 2))
...
def removemin(array, size):
    ...
    array[size-1] = None  # modified by dhnam (to remove dangling references)
    # array = array[:-1]
...
def removemax(array, size):
    ...
    if size == 1:
        elem = array[0]
        array[size-1] = None  # modified by dhnam
    elif size == 2:
        elem = array[1]
        array[size-1] = None  # modified by dhnam
    else:
